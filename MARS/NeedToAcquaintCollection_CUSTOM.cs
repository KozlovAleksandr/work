try
{
	isMobile = tools_web.is_true(SCOPE_WVARS.GetOptProperty("is_mobile"))
	...
	if (isMobile) {
		COLUMNS = ([
			{"data": "name", "title": tools_web.get_web_const( "c_name", curLngWeb ),"type": "link", "sortable": true, "colorsource": "", "width": 150, "minwidth": 150, "click": "OPENWINDOW={url}" },
    		{"data": "object_type_name", "title": tools_web.get_web_const( "c_type", curLngWeb ),"type": "link", "sortable": true, "colorsource": "", "width": 100, "click": "OPENWINDOW={url}"},
    		{"data": "action", "title": tools_web.get_web_const( "vfb_action", curLngWeb ),"type": "link", "sortable": true, "colorsource": "color", "width": 150, "click": "SET=ConfirmAcquaintAction/acquaintAssignID,{id};ACTION=ConfirmAcquaintAction"}
		 ]);
	} else {
    	COLUMNS = ([
    		{"data": "name", "title": tools_web.get_web_const( "c_name", curLngWeb ),"type": "link", "sortable": true, "colorsource": "", "width": "100%", "minwidth": 250, "click": "OPENWINDOW={url}" },
    		{"data": "object_type_name", "title": tools_web.get_web_const( "c_type", curLngWeb ),"type": "link", "sortable": true, "colorsource": "", "width": 200, "click": "OPENWINDOW={url}"},
    		{"data": "action", "title": tools_web.get_web_const( "vfb_action", curLngWeb ),"type": "link", "sortable": true, "colorsource": "color", "width": 200, "click": "SET=ConfirmAcquaintAction/acquaintAssignID,{id};ACTION=ConfirmAcquaintAction"}
    	]);
	}
}
catch(err)
