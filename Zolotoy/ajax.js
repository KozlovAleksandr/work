...
function getNews() {
	$.ajax({
		...
		data:   {
					...
					"parameters" : "iDocumentID=6821888173484432079;bCheckAccess=true;bShowExtendedData=true;sSortType=name;sPeriod=day",
				},
		success: function (data) {
			result = data.results;
			...
			for (i in result) {
				$(".itc-slider-2 .itc-slider__items")
					.append($(`<div class="itc-slider__item">
						...
						`));
				$(".itc-slider__indicators-2")
					.append($(`
						<li class="itc-slider__indicator" data-slide-to="${i}"></li>
					`))
			}
			ItcSlider.createInstances("itc-slider-2")
		},
	})
}
		...
		success: function (data) {
			...
			if (result.newCourses.length > 0) {
				if (result.newCourses.length < 3) {
								...
								<div class="itc-slider__item">
									<div class="new-courses-button-block-item">
										...
										<div class="button-block-item-info">
											...
											<a href="${result.newCourses[a].url}" class="button" target="_blank">Пройти</a>
										</div>			
									</div>
								</div>
							`));
					...
					$(".itc-slider-1 .itc-slider__items")
						.append($("<div class='itc-slider__item'></div>"))
				} else {
					for (a in result.newCourses) {
						$(".itc-slider-1 .itc-slider__items")
							.append($(`
							...
							`));
						$(".itc-slider__indicators-1")
							.append($(`
								<li class="itc-slider__indicator" data-slide-to="${a}"></li>
							`))
					}
					ItcSlider.createInstances("itc-slider-1")
				}
			}
		}
	})
...
class ItcSlider {
  ...
  constructor(el, config = {}, prefix = 'itc-slider__') {
    ...
    this.#config = {
      loop: true, autoplay: false, interval: 0, refresh: true, swipe: true, ...config
    };
...
//   }
  ...
  static createInstances(dataAttr) {
    ...
    Object.keys(dataset).forEach((key) => {
    	...
    	this.getOrCreateInstance(sliderElem, params);
  }
